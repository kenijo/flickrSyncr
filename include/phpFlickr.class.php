...
if ( !class_exists('phpFlickr') ) {
    ...
    class phpFlickr {
        ...
        function request ($command, $args = array(), $nocache = false)
        {
            ...
            if ($this->parsed_response['stat'] == 'fail') {
                if ($this->die_on_error) die("The Flickr API returned the following error: #{$this->parsed_response['code']} - {$this->parsed_response['message']}");
                    ...
                    $this->error = 'Error Code ' . $this->error_code . ': ' . $this->error_msg;
        ...
        function getError () {
            // Returns the error of the last call.  If the last call did not
        ...
        */
        ...
        function photos_getWithoutGeoData ($args = array()) {
            ...
            $this->request('flickr.photos.getWithoutGeoDat', $args);
            return $this->parsed_response ? $this->parsed_response['photos'] : false;
        }
        ...
        /* UNSUPPORTED Methods */
        /* Methods that are not listed on the official API list */
        function collections_create ($title, $description = NULL, $parent_id = NULL) {
            $this->request('flickr.collections.create', array('title'=>$title, 'description'=>$description, 'parent_id'=>$parent_id), TRUE);
            return $this->parsed_response ? $this->parsed_response['collection'] : false;
        }

        function collections_createIcon ($collection_id, $photo_ids = NULL) {
            return $this->call('flickr.collections.createIcon', array('collection_id'=>$collection_id, 'photo_ids'=>$photo_ids));
        }

        function collections_delete ($collection_id, $recursive = NULL) {
            $this->request('flickr.collections.delete', array('collection_id'=>$collection_id, 'recursive'=>$recursive));
            return $this->parsed_response ? true : false;
        }

        function collections_editMeta ($collection_id, $title, $description = NULL) {
            $this->request('flickr.collections.editMeta', array('collection_id'=>$collection_id, 'title'=>$title, 'description'=>$description), TRUE);
            return $this->parsed_response ? true : false;
        }

        function collections_editSets ($collection_id, $photoset_ids, $do_remove = NULL) {
            $this->request('flickr.collections.editSets', array('collection_id'=>$collection_id, 'photoset_ids'=>$photoset_ids, 'do_remove'=>$do_remove), TRUE);
            return $this->parsed_response ? true : false;
        }

        function collections_moveCollection ($collection_id, $parent_collection_id) {
            return $this->call('flickr.collections.moveCollection', array('collection_id'=>$collection_id, 'parent_collection_id'=>$parent_collection_id), TRUE);
        }

        function collections_removeSet ($collection_id, $photoset_id) {
            $this->request('flickr.collections.removeSet', array('collection_id'=>$collection_id, 'photoset_id'=>$photoset_id), TRUE);
            return $this->parsed_response ? true : false;
        }

        function collections_sortCollections ($collection_id, $child_collection_ids = NULL, $no_move = NULL) {
            return $this->call('flickr.collections.sortCollections', array('collection_id'=>$collection_id, 'child_collection_ids'=>$child_collection_ids, 'no_move'=>$no_move), TRUE);
        }

        function collections_getInfosuggestIconPhotos ($collection_id) {
            return $this->call('flickr.collections.suggestIconPhotos', array('collection_id'=>$collection_id));
        }
    }
}
